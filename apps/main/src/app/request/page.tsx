...
export default function RequestFormPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok || !data.success) throw new Error(data.message || `Submission failed (${res.status})`);
      router.push("/request/success");
    } catch (err) {
      const msg = err instanceof Error ? err.message : "Submission failed";
      setSubmitError(
        msg.includes("fetch") || msg.includes("Failed")
          ? "Cannot reach API server. Run: npm run dev:server"
          : msg
      );
    } finally {
      setSubmitting(false);
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_rgba(45,212,191,0.1),transparent_55%)]" />

      <header className="relative border-b border-border/50 bg-background/80 backdrop-blur-xl safe-top">
        <div className="max-w-4xl mx-auto px-3 sm:px-4 h-14 sm:h-16 flex items-center justify-between gap-2">
          <Logo variant="navbar" />
          <Link href="/" className="text-xs sm:text-sm text-muted-foreground hover:text-foreground flex items-center gap-1 shrink-0 min-h-[44px]">
            <ArrowLeft className="w-4 h-4" /> <span className="hidden min-[400px]:inline">Back to Home</span><span className="min-[400px]:hidden">Back</span>
          </Link>
        </div>
      </header>

      <main className="relative max-w-4xl mx-auto px-3 sm:px-4 py-6 sm:py-10 pb-10 safe-bottom">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8 sm:mb-10">
          <h1 className="text-2xl sm:text-4xl font-bold tracking-tight text-balance px-2">
        ...
        <div className="mb-8 sm:mb-10">
          <div className="flex justify-between gap-1 sm:gap-2 mb-4">
            {STEPS.map((s) => (
              <div key={s.id} className={cn("flex flex-col sm:flex-row items-center gap-1 sm:gap-2 flex-1 min-w-0", step >= s.id ? "text-primary" : "text-muted-foreground")}>
                <div className={cn(
                  "w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center border-2 transition-all shrink-0",
                ...
                <span className="text-[10px] sm:text-sm font-medium text-center leading-tight">{s.title}</span>
              </div>
            ))}
          </div>
        ...
        <div className="rounded-2xl border border-border/50 bg-card/80 backdrop-blur-sm p-4 sm:p-8 shadow-xl">
            ...
            {step === 2 && (
              <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 min-[480px]:grid-cols-2 sm:grid-cols-3 gap-2 mt-2">
                    {FEATURES.map((f) => (
                ...
                <div>
                  ...
                  {files.length > 0 && (
                    <div className="mt-4 space-y-2">
                      {files.map((file, i) => (
                        <div key={i} className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-3 rounded-xl bg-muted/50">
          ...
          <div className="flex flex-col-reverse sm:flex-row sm:justify-between gap-3 mt-8 pt-6 border-t border-border">
            <button
              type="button"
              onClick={prev}
              disabled={step === 1}
              className="inline-flex items-center justify-center px-5 py-3 rounded-xl border border-border text-sm font-medium disabled:opacity-40 hover:bg-muted transition-colors min-h-[44px] w-full sm:w-auto touch-manipulation"
            >
            ...
            {step < 3 ? (
              <button
                type="button"
                onClick={next}
                className="inline-flex items-center justify-center px-6 py-3 rounded-xl bg-gradient-to-r from-teal-500 to-violet-600 text-white text-sm font-medium hover:opacity-90 transition-opacity min-h-[44px] w-full sm:w-auto touch-manipulation"
              >
              ...
              <button
                ...
                disabled={submitting}
                className="inline-flex items-center justify-center px-6 py-3 rounded-xl bg-gradient-to-r from-teal-500 to-violet-600 text-white text-sm font-medium hover:opacity-90 transition-opacity disabled:opacity-60 min-h-[44px] w-full sm:w-auto touch-manipulation"
              >
